...
public:
     ...
     */
    std::map<std::string, float> get_statistics() {
        std::map<std::string, float> stat_map;
        auto stats = orchestrator_->getStats();
        ...
        for (const auto& [module_name, activity] : stats.module_activities) {
            stat_map["module_" + module_name] = activity;
        }
