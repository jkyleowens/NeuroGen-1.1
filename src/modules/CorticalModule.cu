...
CorticalModule::CorticalModule(const Config& config, int gpu_device_id) 
    ...
    // Disable memory pool to prevent OOM with multiple modules
    cuda_config.enable_memory_pool = false;
    ...
    net_config.num_synapses = static_cast<size_t>(
        std::max(1, config.num_neurons) *
        std::max(1, config.fanout_per_neuron));
    ...
    net_config.num_outputs = (config.num_outputs > 0)
        ...
        : static_cast<size_t>(std::max(1, config.num_neurons / 4));
    net_config.connection_density = static_cast<float>(net_config.num_synapses) /
        static_cast<float>(std::max<size_t>(1, static_cast<size_t>(config.num_neurons) * static_cast<size_t>(config.num_neurons)));
    ...
    std::cout << "✓ Initialized module: " << config_.module_name 
...
void CorticalModule::update(float dt_ms, float reward_signal) {
    ...
    neural_engine_->update(dt_ms, modulated_reward, 0.0f);
...
void CorticalModule::setPlasticity(bool enabled) {
    // This would need a method in NetworkCUDA to enable/disable learning
    // For now, we can control it through the reward signal in update()
...
NetworkCUDA* CorticalModule::getNeuralEngine() {
    return neural_engine_.get();
}

const NetworkCUDA* CorticalModule::getNeuralEngine() const {
    return neural_engine_.get();
}
