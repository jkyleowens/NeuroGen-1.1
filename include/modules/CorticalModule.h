...
// Forward declaration to avoid CUDA header dependency in modules
class NetworkCUDA;
...
public:
    ...
    // Returns raw pointer to incomplete type (safe if only used for pointer storage)
    NetworkCUDA* getNeuralEngine();
    const NetworkCUDA* getNeuralEngine() const;

    // 12. Persistence: Access neuron/synapse state via wrapper
    // These methods delegate to NetworkCUDA implementation without exposing the type
